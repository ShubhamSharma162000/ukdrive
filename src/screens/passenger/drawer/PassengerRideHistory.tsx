...
export default function PassengerRideHistory({ navigation }: any) {
  ...
  useEffect(() => {
    ...
    const wsUrl = 'https://wordier-granville-driftingly.ngrok-free.dev/ws';

    const socket = new WebSocket(wsUrl);
    socketRef.current = socket;

    socket.onopen = () => {
      console.log('for rides history Connected to WebSocket');
      socket.send(
        JSON.stringify({
    ...
    socket.onmessage = event => {
      ...
      if (msg.type === 'ride_history_data') {
        console.log('📦 Ride history received:', msg.data);
        setRides(msg.data);
      }
    };
                ...
                <TouchableOpacity
                  onPress={() =>
                    navigation.navigate('PassengerTabs', {
                      screen: 'HomeStack',
                      params: { screen: 'DetailHistoryScreen' },
                    })
                  }
                  style={{
